...
export function registerBrowserTools(gw: Gateway): void {
  ...
  r.register({ name: "browser_upload_file", description: "Set a local file on a file input selector.", risk: 3, inputSchema: z.object({ selector: z.string(), path: z.string() }), summarize: () => 'browser file', handler: async (i, ctx) => { const s = ensure(ctx); const file = gw.pathGuard.assertAccess(i.path, "read"); await s.page!.setInputFiles(i.selector, file); return { selector: i.selector, path: file }; } });
